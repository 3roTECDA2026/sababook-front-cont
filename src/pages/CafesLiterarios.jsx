...
export default function CafesLiterarios() {
  ...
  const cargarCafes = async (signal) => {
    try {
      setLoading(true);
      const data = await getCafesLiterarios(user?.usuario_id, signal);
      setCafes(data);
      setError(null);
    } catch (err) {
  ...
  const [cargandoAsistencia, setCargandoAsistencia] = useState(null);

  // Confirmar o cancelar (Toggle) asistencia con lógica optimista
  const handleAsistencia = async (cafeId, confirmar) => {
    if (!user?.usuario_id) {
  ...
  const handleVoto = async (cafeId, votoPositivo) => {
    if (!user?.usuario_id) {
  ...
  const esDocenteOAdmin = user?.rol_id === 2 || user?.rol_id === 3 || user?.rol === 'docente' || user?.rol === 'administrador';
            ...
            return (
              ...
              >
                ...
                >
                  <Box>
                    ...
                    <Typography variant="caption" sx={{ opacity: 0.9, display: 'flex', alignItems: 'center', gap: 0.5 }}>
                      <EventIcon fontSize="inherit" /> {fechaFormateada} | <LocationOnIcon fontSize="inherit" /> {cafe.lugar}
                    </Typography>
                  </Box>
                ...
                <CardContent sx={{ p: 3 }}>
                  ...
                  <Typography variant="subtitle2" fontWeight="bold" color="text.primary" mb={1} display="flex" alignItems="center" gap={0.5}>
